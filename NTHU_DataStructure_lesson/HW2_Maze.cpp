...
{ // ��X�g�c���@�Ӹ��|�]�p�G�����ܡ^�F maze[0][i] = maze[m+1][i] = maze[j][0] = maze[j][p+1] = 1, 0 �T i �T p+1, 0 �T j �T m+1�C�q (1, 1) �}�l
    mark[1][1] = 1;
    ...
    { // ���|���O�Ū�
        ...
        while (d < 8)
        { // ���e����
            int g = i + movedir[d].di;
            int h = j + movedir[d].dj;
            if ((g == m) && (h == p))
            {                  // ��F�X�f
                cout << stack; // ��X���|
                cout << "(" << i << ", " << j << ", ";
...
int main(void)
{
    ...
    { // edge
        ...
        {
            if (i == 0 || i == m + 1)
                maze[i][j] = 1; // read in the maze
            else if (j == 0 || j == p + 1)
                maze[i][j] = 1;
            else
                fscanf(MAZE, "%d", &maze[i][j]);
        }
    }
